...
At spawn, Supervisor MUST explicitly restrict the CPA child environment to OS settings: `PATH`, `TMP`, and `TEMP`; additionally `HOME` and `TMPDIR` on Unix, or `SystemRoot`, `WINDIR`, `USERPROFILE`, `HOMEDRIVE`, and `HOMEPATH` on Windows. Windows variable names are case insensitive. An empty selection MUST NOT fall back to parent environment inheritance. Runtime credentials and control-plane configuration, including all `CPAMP_*` variables, MUST NOT be passed to CPA. Other application credentials, proxy settings, and arbitrary parent variables are not forwarded; any future CPA environment provisioning requires a separate explicit contract. The working directory remains inherited.
